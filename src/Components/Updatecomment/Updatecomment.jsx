  ...
  async function handleupdatecomment(values){
    ...
    .then((res) => {
      if(res.data.message == "success"){
        console.log(res);
        toast.success("Update comment is done successfully")
        queee.invalidateQueries({queryKey : ["getuserpost"]})
        queee.invalidateQueries({queryKey : ["getprofile"]})
      }
    })
    .catch((err) => {
      console.log(err);
      toast.error("Update comment is failed")
    })
  }
                  ...
                  <div className="p-4 md:p-5">
                      <form className="space-y-4" action="#">
                          <div>
                              ...
                              <input type="text" {...register("content")} id="text" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder=""/>
                          </div>
                          <button type="submit" onClick={handleSubmit(handleupdatecomment)} className="w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Update comment</button>
                      </form>
                  </div>
              </div>
          </div>
